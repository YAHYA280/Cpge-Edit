...
export default function PreAdmissionLetterUploadArea({ parameters, customer_id, isOpen, onOpenChange }) {

    const [adminData, setAdminData] = useCookies('admin_data')


    const [preAdmissionLetter, setPreAdmissionLetter] = useState(null)
    ...
    function setState(e, target) {
        ...
        if (target == 'pre_admission_letter') {
            setPreAdmissionLetter(e)
        }
    ...
    async function submitStep4Details() {
        ...
        if (!preAdmissionLetter || preAdmissionLetter.length == 0) {
            isValid = false
        }
        ...
        if (isValid) {
            ...
            try {
                ...
                formData.append('pre_admission_letter', preAdmissionLetter[0].file)
                    ...
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col text-center items-center gap-2 justify-center">
                                ...
                                <p>Upload pre-admission letter file</p>
                            </ModalHeader>
                            <ModalBody>
                                <div className="flex flex-col gap-5">
                                    ...
                                    <HtmlInput
                                        ...
                                        placeholder={{ placeholder: 'Pre-admission letter', icon: <SchoolIcon /> }}
                                        onChange={(files) => setState(files, 'pre_admission_letter')}
                                    />



                                    {
                                        preAdmissionLetter && preAdmissionLetter.length > 0 &&
                                        ...
                                        <div className="flex items-center justify-between p-2 border-1 rounded-xl bg-light-2 ">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="max-w-[250px]">
                                                    <p className="text-sm sm:text-base font-medium truncate">{preAdmissionLetter[0]?.fileName}</p>
                                                    <span className="text-xs sm:text-sm flex items-center gap-1">
                                                        <p className="flex items-center gap-1">
                                                            {preAdmissionLetter[0]?.extension.toUpperCase()}
                                                        </p>
                                                        <span className="h-1 w-1 rounded-full bg-dark-3" />
                                                        <p className="flex items-center gap-1">
                                                            {calculateFileSize(preAdmissionLetter[0]?.size)}
                                                        </p>
                                                    </span>
